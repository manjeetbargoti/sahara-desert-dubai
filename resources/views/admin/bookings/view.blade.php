...
<div class="nk-block-head">
    ...
    <div class="nk-block-between g-3">
        <div class="nk-block-head-content">
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#updateBookingForm">Update Booking</button>
        </div>
        <div class="nk-block-head-content">
            <p class="text-muted">Vendor: <span class="text-primary fw-bold">{{ @$booking->vendor->name ?? '--' }}</span></p>
        </div>
    </div>
    @endif
</div><!-- .nk-block-head -->
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.bookings.update', @$booking->id) }}" method="POST" class="form-validate is-alter">
                    @csrf
                    <div class="form-group">
                        <label class="form-label" for="customerName">Customer Name</label>
                        <div class="form-control-wrap">
                            <input type="text" name="name" value="{{ @$booking->name }}" class="form-control" id="customerName" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="customerEmail">Customer Email</label>
                        <div class="form-control-wrap">
                            <input type="email" name="email" value="{{ @$booking->email }}" class="form-control" id="customerEmail" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="customerPhone">Customer Phone</label>
                        <div class="form-control-wrap">
                            <input type="text" name="phone" value="{{ @$booking->phone }}" class="form-control" id="customerPhone">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-control-wrap">
                            <select name="vendor" class="form-select form-control @error('vendor') is-invalid @enderror" data-search="off" id="assignVendor" data-placeholder="Please select vendor">
                                <option value=""></option>
                                @if(!empty($vendors))
                                @foreach($vendors as $key => $vendor)
                                <option value="{{ $vendor->id }}" {{ $vendor->id == $booking->vendor_id ? 'selected' : ''}}>{{ $vendor->name }}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-control-wrap">
                            <select name="status" class="form-select form-control @error('status') is-invalid @enderror" data-search="off" id="bookingStatus" data-placeholder="Please select booking status">
                                <option value=""></option>
                                <option value="1" {{ $booking->status == 1 ? 'selected' : '' }}>Fullfilled</option>
                                <option value="2" {{ $booking->status == 2 ? 'selected' : '' }}>UnFullfilled</option>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-control-wrap">
                            <select name="payment_status" class="form-select form-control @error('payment_status') is-invalid @enderror" data-search="off" id="paymentStatus" data-placeholder="Please select payment status">
                                <option value=""></option>
